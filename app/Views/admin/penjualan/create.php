                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" id="kdpenjualan" name="kdpenjualan" readonly>
                        </div>
                        <div class="form-group mb-3">
                            <label for="tglpenjualan" class="form-label">Tanggal Penjualan</label>
                            <input type="date" class="form-control" id="tglpenjualan" name="tglpenjualan" required value="<?= date('Y-m-d') ?>">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
...
<script>
    $(document).ready(function() {
        ...
        // Fungsi untuk mendapatkan kode penjualan berikutnya
        function getNextKdPenjualan() {
            $.ajax({
                url: '<?= site_url('admin/penjualan/getNextKdPenjualan') ?>',
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    if (response.status === 'success') {
                        $('#kdpenjualan').val(response.data.kdpenjualan);
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Error saat mengambil kode penjualan: ', error);
                }
            });
        }
        ...
        $('#form-penjualan').submit(function(e) {
            ...
            $.ajax({
                url: '<?= site_url('admin/penjualan/add') ?>',
                type: 'POST',
                data: $(this).serialize(),
                dataType: 'json',
                success: function(response) {
