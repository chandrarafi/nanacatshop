        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="table-fasilitas">
                        <thead>
                            <tr>
                                <th>No</th>
...
<script>
    $(document).ready(function() {
        ...
        function loadKategori(keyword = '') {
            $.ajax({
                url: '<?= site_url('admin/laporan/fasilitas/kategori') ?>',
                type: 'GET',
                dataType: 'json',
        ...
        function renderKategoriTable(keyword = '') {
            ...
            if (filteredData.length === 0) {
                $('#table-kategori tbody').html('<tr><td colspan="2" class="text-center">Tidak ada data</td></tr>');
            } else {
                $.each(filteredData, function(i, item) {
                    const row = `
                        <tr>
                            <td>${item.kategori}</td>
                            <td>
                                <button class="btn btn-sm btn-primary btn-pilih-kategori" data-kategori="${item.kategori}">
                                    Pilih
                                </button>
                            </td>
                        </tr>
        ...
        function loadData() {
            ...
            $.ajax({
                url: '<?= site_url('admin/laporan/fasilitas/data') ?>',
                type: 'GET',
                data: {
                    kategori: kategori,
                    tgl_awal: tglAwal,
                ...
                success: function(response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        if (response.data.length === 0) {
                            $('#table-fasilitas tbody').html('<tr><td colspan="8" class="text-center">Tidak ada data</td></tr>');
                        } else {
        ...
        $('#btn-cetak').click(function() {
            ...
            let url = '<?= site_url('admin/laporan/fasilitas/cetak') ?>';
            let params = [];
