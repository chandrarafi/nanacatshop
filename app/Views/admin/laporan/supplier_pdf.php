...
<body>
    ...
    <div class="info">
        <?php if (!empty($filter['keyword'])): ?>
            <div class="info-item">
                <strong>Kata Kunci:</strong> <?= safeOutput($filter['keyword']) ?>
            </div>
        <?php endif; ?>
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                ?>
                    <tr>
                        <td><?= $no++ ?></td>
                        <td><?= safeOutput($s->kdspl) ?></td>
                        <td><?= safeOutput($s->namaspl) ?></td>
                        <td><?= safeOutput($s->nohp) ?></td>
                        <td><?= safeOutput($s->email) ?></td>
                        <td><?= safeOutput($s->alamat) ?></td>
                        <td><?= date('d-m-Y', strtotime($s->created_at)) ?></td>
                    </tr>
                <?php endforeach; ?>
                <tr>
                    <th colspan="6" class="text-right">Total Supplier</th>
                    <th><?= count($supplier) ?></th>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
